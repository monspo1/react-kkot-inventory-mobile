...
const HomeScreenContent = ({ navigation }) => {
  ...
  const [visibleAddNewBoxModal, setVisibleAddNewBoxModal] =
  ...
  useEffect(() => {
    ...
    dispatch(setNewBoxInitial(''))
    setTextForNewBoxInitial('');
  ...
  const handleNewBoxInitialTextChange = (boxInitial) => {
    // 대문자로 변환하고, 영문자와 숫자만 남깁니다.
    const upperCaseText = boxInitial.toUpperCase().replace(/[^A-Z0-9]/g, "");
    ...
    if (match) {
      ...
      setTextForNewBoxInitial(newText);
      setNewBoxInitialError(!newBoxInitialRegex.test(newText));
    } else {
      setTextForNewBoxInitial(upperCaseText);
      setNewBoxInitialError(true);
    }
    ...
    <>
      ...
      >
        ...
        {boxesData.length === 0 ? (
          ...
          boxesData.map((elem, idx) => (
            <BoxListElem elem={elem} key={`key-${idx}`} idx={idx} />
          ))
        )}
      ...
      >
        ...
        <TextInput
          label="Enter a box initial"
          mode="outlined"
          style={styles.addNewBoxModalTextInput}
          value={textForNewBoxInitial}
          onChangeText={(text) =>
            handleNewBoxInitialTextChange(text.toUpperCase())
          }
          error={newBoxInitialError}
          placeholder="(e.g, AB-01 or BBB-99)"
        />
        <HelperText
          type="error"
          visible={newBoxInitialError}
          style={styles.addNewBoxModalErrorMessage}
        >
          NOT Valid! Enter 2 or 3 letters, followed by a hyphen, then a number
          from 0 to 99 (e.g, AB-01 or BBB-77)
        </HelperText>
        ...
        <View style={styles.horizontalButtonContainer}>
          ...
          <Button
            icon="plus-thick"
            mode="contained-tonal"
            disabled={shouldDisableCreateNewBoxBtn()}
            labelStyle={styles.bigButtonLabelStyle}
            style={[styles.horizontalButton, styles.bigButtonStyle]}
            onPress={submitCreateNewBoxModal}
          >
            CREATE
          </Button>
        </View>
